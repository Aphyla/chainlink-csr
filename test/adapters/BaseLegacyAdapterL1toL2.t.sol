...
contract BaseLegacyAdapterL1toL2Test is Test {
    MockReceiver public receiver;
    BaseLegacyAdapterL1toL2 public adapter;
    ...
    function setUp() public {
        ...
        adapter = new BaseLegacyAdapterL1toL2(address(l1ERC20Bridge), address(receiver));

        receiver.setAdapter(address(adapter));
    }

    function test_Constructor() public {
        adapter = new BaseLegacyAdapterL1toL2(address(l1ERC20Bridge), address(receiver)); // to fix coverage
    ...
    function test_Revert_Constructor() public {
        vm.expectRevert(BaseLegacyAdapterL1toL2.BaseLegacyAdapterL1toL2InvalidParameters.selector);
        adapter = new BaseLegacyAdapterL1toL2(address(0), address(receiver));

        vm.expectRevert(IBridgeAdapter.BridgeAdapterInvalidParameters.selector);
        adapter = new BaseLegacyAdapterL1toL2(address(l1ERC20Bridge), address(0));
    }
    ...
    function test_Fuzz_SendToken(uint256 amount, uint32 l2Gas) public {
        ...
        assertEq(
            ...
            abi.encodeWithSelector(
                IBaseL1ERC20TokenBridge.depositERC20To.selector,
                l1Token,
                l2Token,
                recipient,
                amount,
    ...
    function test_Fuzz_Revert_SendToken(address msgSender, uint128 amount, uint32 l2Gas) public {
        ...
        vm.expectRevert(abi.encodeWithSelector(BaseLegacyAdapterL1toL2.BaseLegacyAdapterL1toL2InvalidFeeToken.selector));
        receiver.sendToken(uint64(0), recipient, amount, feeData);

        feeData = abi.encodePacked(amount, false, uint32(0));

        vm.expectRevert(
            abi.encodeWithSelector(BaseLegacyAdapterL1toL2.BaseLegacyAdapterL1toL2InvalidFeeAmount.selector, amount, 0)
        );
        receiver.sendToken(uint64(0), recipient, amount, feeData);
