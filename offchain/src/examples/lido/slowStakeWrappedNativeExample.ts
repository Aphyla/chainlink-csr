...
/**
 * Example: Execute slowStake with wrapped native token (WETH) for staking and native ETH for CCIP fees.
 * This demonstrates the hybrid flow of slow staking where:
 * - Staking amount is paid in wrapped native token (WETH)
 * - CCIP fees are paid in native ETH
 * - Bridge fees are paid in native ETH
 * - Requires approval only for WETH tokens (handled automatically)
 */
async function runExample(): Promise<void> {
  ...
  console.log(
    `🐌 SlowStake with Wrapped Native + Native CCIP on ${network.name}`
  );
  ...
  try {
    ...
    const ccipFeePaymentMethod = 'native';
    ...
    console.log(
      `🔗 CCIP fee payment: ${ccipFeePaymentMethod.toUpperCase()} ETH`
    );
    console.log(
    ...
    console.log(
      `  ETH Required: ${feeEstimation.summary.ethRequiredFormatted} ETH (CCIP + bridge fees)`
    );
    console.log('');

    // Execute the slowStake transaction
    console.log('🔄 Executing slowStake with wrapped native + native CCIP...');
    ...
    console.log(
      `  Total ETH Value: ${formatEther(result.contractCall.totalValue)} ETH (CCIP + bridge fees)`
    );
    console.log('');
    ...
    console.log(`  Staking Amount: ${formatEther(stakingAmount)} WETH`);
    console.log(
      `  CCIP Fee (O→D): ${result.feeEstimation.summary.feeOtoDFormatted} ETH`
    );
    console.log(
      `    └─ Max Fee: ${formatEther(result.feeEstimation.feeOtoD.breakdown.maxFee)} ETH`
    );
    console.log(
    ...
    console.log(`  This Method (WETH + Native CCIP):`);
    ...
    console.log(
      `  Alternative (WETH + LINK): Would require WETH + ${result.feeEstimation.summary.linkRequiredFormatted} LINK + bridge ETH`
    );
    console.log('');
    ...
    console.log(
      '  2. 🌉 ETH will be bridged to Ethereum via CCIP (paid with native ETH)'
    );
    console.log(
