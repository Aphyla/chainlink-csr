...
/**
 * Example: Execute slowStake with native ETH payment for both staking and CCIP fees.
 * This demonstrates the full flow of slow staking where:
 * - Staking amount is paid in native ETH
 * - CCIP fees are paid in native ETH
 * - Bridge fees are paid in native ETH
 * - Single ETH payment covers all costs
 */
async function runExample(): Promise<void> {
  const chainKey = BASE_MAINNET;
  const network = getNetworkConfig(chainKey);

  console.log(`🐌 SlowStake with Native ETH on ${network.name}`);
  ...
  try {
    ...
    console.log(`🔗 CCIP fee payment: ${ccipFeePaymentMethod} ETH`);
    console.log('');
    ...
    console.log(
      `  Total ETH Required: ${feeEstimation.summary.ethRequiredFormatted} ETH`
    );
    console.log('');

    // Execute the slowStake transaction
    console.log('🔄 Executing slowStake with native ETH...');
    const result = await executeSlowStake({
      ...
      autoApproveUnlimited: false, // Not needed for native payments
    });
    ...
    console.log(
      `  Total Value Sent: ${formatEther(result.contractCall.totalValue)} ETH`
    );
    console.log('');
    ...
    console.log('  1. 🌉 Your ETH will be bridged to Ethereum via CCIP');
    console.log(
