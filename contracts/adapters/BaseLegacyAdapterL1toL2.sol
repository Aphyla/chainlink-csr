...
import {IBaseL1ERC20TokenBridge} from "../interfaces/IBaseL1ERC20TokenBridge.sol";

/**
 * @title BaseLegacyAdapterL1toL2 Contract
 * @dev A bridge adapter for sending tokens from L1 to L2 on Base.
 */
contract BaseLegacyAdapterL1toL2 is BridgeAdapter {
    using SafeERC20 for IERC20;

    /* Error thrown when the fee amount is invalid */
    error BaseLegacyAdapterL1toL2InvalidFeeAmount(uint256 expectedFeeAmount, uint256 feeAmount);
    error BaseLegacyAdapterL1toL2InvalidFeeToken();
    error BaseLegacyAdapterL1toL2InvalidParameters();
    ...
    constructor(address l1ERC20Bridge, address delegator) BridgeAdapter(delegator) {
        if (l1ERC20Bridge == address(0)) revert BaseLegacyAdapterL1toL2InvalidParameters();

        L1_ERC20_BRIDGE = l1ERC20Bridge;
        L1_TOKEN = IBaseL1ERC20TokenBridge(l1ERC20Bridge).l1Token();
        L2_TOKEN = IBaseL1ERC20TokenBridge(l1ERC20Bridge).l2Token();
    }
    ...
    function _sendToken(uint64, address to, uint256 amount, bytes calldata feeData) internal override {
        ...
        if (payInLink) revert BaseLegacyAdapterL1toL2InvalidFeeToken();
        if (feeAmount != 0) revert BaseLegacyAdapterL1toL2InvalidFeeAmount(feeAmount, 0);

        IERC20(L1_TOKEN).forceApprove(L1_ERC20_BRIDGE, amount);

        IBaseL1ERC20TokenBridge(L1_ERC20_BRIDGE).depositERC20To(L1_TOKEN, L2_TOKEN, to, amount, l2Gas, new bytes(0));
